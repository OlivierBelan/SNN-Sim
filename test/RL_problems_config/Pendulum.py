...
class Pendulum(Config_Problem):
    ...
    def get_env(self, render:bool=False) -> Environment_Gym:
        if render == True:
            ...
            self.gym_env:gym.Env = gym.make("Pendulum-v1")
    ...
    def decoding_output_network_to_action(self, action:np.ndarray, network_type:str) -> np.ndarray:        
        if network_type == "SNN":
            return np.interp(action, [0, 1], [-2, +2])
        else:
            return np.interp(action, [-1, 1], [-2, +2]) # car l'action est déjà entre -1 et 1 (tanh function output)
