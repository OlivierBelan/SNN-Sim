...
def normalize_number(nb:float, max_value:float, min_value:float) -> float:
    return (nb - min_value) / (max_value - min_value)

class Car_Racing(Config_Problem):
    def __init__(self, name:str, config_path:str, nb_input:int=27648, nb_output:int=5, obs_max_init_value:float=5, obs_min_init_value:float=-5, termination_finess:float=None, auto_obersvation:bool=False, auto_obersvation_ratio:float=0.01) -> None:
    ...
    def get_env(self, render:bool=False) -> Environment_Gym:
        if render == True:
            self.gym_env:gym.Env = gym.make("CarRacing-v2", domain_randomize=False, continuous=False, render_mode="human")
        else:
            self.gym_env:gym.Env = gym.make("CarRacing-v2", domain_randomize=False, continuous=False)
    ...
    @staticmethod
    @nb.njit(cache=True, fastmath=False, nogil=True)
    def encoding_observation_to_input_network(observation: np.ndarray, observation_max:np.ndarray, observation_min:np.ndarray) -> np.ndarray:
        observation_len:int = len(observation)
        observation = observation.flatten()
        for i in nb.prange(observation_len):
            observation[i] = normalize_number(observation[i], 255.0, 0.0)
