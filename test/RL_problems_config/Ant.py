...
class Ant(Config_Problem):
    ...
    def get_env(self, render=False) -> Environment_Gym:
        if render == True:
            self.gym_env:gym.Env = gym.make("Ant-v4", render_mode="human")
        else:
            self.gym_env:gym.Env = gym.make("Ant-v4")
