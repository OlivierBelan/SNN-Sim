...
class Acrobot_Config(Config_Problem):
    ...
    def get_env(self, render:bool=False) -> Environment_Gym:
        if render == True:
            self.gym_env:gym.Env = gym.make('Acrobot-v1', render_mode="human")
        else:
            self.gym_env:gym.Env = gym.make('Acrobot-v1')
    ...
    @staticmethod
    def decoding_output_network_to_action(action: np.ndarray, network_type:str) -> int:
