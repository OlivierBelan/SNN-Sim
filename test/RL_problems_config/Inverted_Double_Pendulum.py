...
class Inverted_Double_Pendulum(Config_Problem):
    def __init__(self, name:str, config_path:str, nb_input:int=11, nb_output:int=1, obs_max_init_value:float=5, obs_min_init_value:float=-5, termination_finess:float=None, auto_obersvation:bool=True, auto_obersvation_ratio:float=0.01) -> None:
    ...
    def get_env(self, render:bool=False) -> Environment_Gym:
        if render == True:
            self.gym_env:gym.Env = gym.make("InvertedDoublePendulum-v4", render_mode="human")
        else:
            self.gym_env:gym.Env = gym.make("InvertedDoublePendulum-v4")
